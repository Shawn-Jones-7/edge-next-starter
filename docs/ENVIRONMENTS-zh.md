...
| `SIGNING_SECRET`        | 签名密钥（用于文件下载等签名 URL），使用 `wrangler secret put SIGNING_SECRET` 设置 |

如有额外的第三方服务密钥，请在此处新增并在 `wrangler.*.toml` 的 `[vars]` 中引用。

### SIGNING_SECRET 配置说明

`SIGNING_SECRET` 用于生成和验证签名 URL（如文件下载链接）。
...
- 本地开发：在 `.dev.vars` 中使用默认值 `dev-secret`
- 测试/生产：使用 `wrangler secret put SIGNING_SECRET` 设置
